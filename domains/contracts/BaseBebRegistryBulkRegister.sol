...
import "./BaseBebRegistryOneStepController.sol";
import "@openzeppelin/contracts/access/Ownable.sol";

contract BaseBebRegistryBulkRegister is Ownable {
    BaseBebRegistryOneStepController public controller;

    constructor(address _controllerAddress) {
        controller = BaseBebRegistryOneStepController(_controllerAddress);
    }
    ...
    function bulkRegister(
        ...
        require(
            ...
            "BaseBebRegistryBulkRegister: Input arrays must have the same length"
        );

        uint256 totalCost = calculateTotalPrice(names, durations);
        require(msg.value >= totalCost, "BaseBebRegistryBulkRegister: Insufficient funds");
