 ...
 */
contract BaseBebRegistryOneStepController is Ownable, IBebRegistryOneStepController {
    ...
    function valid(string memory name) public pure returns (bool) {
        return name.strlen() >= 10 && _compareStrings(_trim(name, 0, 5), "base_");
    }
    ...
    function register(
        ...
        require(
            valid(name),
            "BaseBebRegistryOneStepController: Invalid name!"
        );
        bytes32 label = keccak256(bytes(name));
        IPriceOracle.Price memory price = rentPrice(name, duration);
        require(
            msg.value >= (price.base + price.premium),
            "BaseBebRegistryOneStepController: Not enough ether provided"
        );
        ...
        override
    {
        ...
        require(
            msg.value >= (price.base + price.premium),
            "BaseBebRegistryOneStepController: Not enough Ether provided for renewal"
        );
