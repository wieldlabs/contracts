 ...
 */
contract OPBebRegistryOneStepController is Ownable, IBebRegistryOneStepController {
    ...
    function valid(string memory name) public pure returns (bool) {
        return name.strlen() >= 10 && _compareStrings(_trim(name, 0, 3), "op_");
    }
    ...
    function register(
        ...
        require(
            valid(name),
            "OPBebRegistryOneStepController: Invalid name!"
        );
        bytes32 label = keccak256(bytes(name));
        IPriceOracle.Price memory price = rentPrice(name, duration);
        require(
            msg.value >= (price.base + price.premium),
            "OPBebRegistryOneStepController: Not enough ether provided"
        );
        ...
        override
    {
        ...
        require(
            msg.value >= (price.base + price.premium),
            "OPBebRegistryOneStepController: Not enough Ether provided for renewal"
        );
